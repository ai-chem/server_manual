...
Вам дают доступ к [тесту](https://forms.gle/8qTtL5nsLmYRr1xX8), вы его проходите. После выполнения теста необходимо написать ответственному. Если всё в порядке, вам откроют доступ на сервер и пришлют username и пароль ✅
...
💡 Как подключиться? 
   ...
   - Введите команду для подключения в формате: `ssh your_login@77.234.216.102 -i путь/к/ключу`, например `ssh aorlova@77.234.216.102 -i /home/aorlova/.ssh/id_rsa`
...
💡 Как подключиться? 
   - Введите команду `ssh your_login@77.234.216.102`, где `your_login` — ваш логин от кластера.
